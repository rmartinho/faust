...
- Create a text file named `faust.yml` and enter the following as the contents:
  ...
  There are more options; if you are curious you can look at a more [complete example](https://github.com/rmartinho/faust/blob/main/examples/faust.yml).
...
- run `faust -s path/to/faust.yml` in a terminal (obviously replace that with the correct path to the `faust.yml` file).

This will parse the mod folder and output all of the site files into a folder named `site` next to the `faust.yml` file. If no errors occur,
it will also start a small web server that lets you browse the site locally right away.
...
The generated site is fully self-contained and doesn't depend on any external resources.
